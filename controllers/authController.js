exports.register = async (req, res) => {
    try {
      const { name, email, password, phone, role, location, availableBooks, anonymous } = req.body;
      ...
      if (role === "school" && (!location || !availableBooks)) {
        return res.status(400).json({ message: "Schools must provide location and available books" });
      }
  
      if (role === "donor" && typeof anonymous !== "boolean") {
        return res.status(400).json({ message: "Donors must specify anonymous as true or false" });
      }
